...
class Table():
    ...
    def __str__(self):
        ...
        string += "{}[{}{}{}]\n".format(round(ws_count) * ' ', const.COLOR_BOLD_GREEN, self.__table_name, const.COLOR_RESET)
        string += "+" + (rowsize-2) * '-' + "+\n"

        for key, group in groupby(self.__rows, lambda x: x.number()):
            for row in group:
                row_str = "| {} | {}".format(str(row.number_colored()), row.name_colored())
                ws_count = rowsize - (row.number_length() + row.name_length() + 6) # 6 = template_size (5) + ending_pipe (1)
